  ...
  return (
    <div className="grid gap-10 md:grid-cols-2">
      <div className="rounded-2xl border border-white/10 bg-white/5 p-6 text-white">
        <h3 className="mb-4 font-serif text-2xl">Reserve a Table</h3>
        <form onSubmit={submit} className="grid gap-4">
          <div>
            <label className="mb-1 block text-sm text-white/70">Name</label>
            <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required className="w-full rounded-xl border border-white/20 bg-white/10 px-4 py-2 text-white placeholder-white/40 outline-none focus:ring-2 focus:ring-rose-500/40" placeholder="Your full name" />
          </div>
          <div>
            <label className="mb-1 block text-sm text-white/70">Email</label>
            <input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} required className="w-full rounded-xl border border-white/20 bg-white/10 px-4 py-2 text-white placeholder-white/40 outline-none focus:ring-2 focus:ring-rose-500/40" placeholder="you@example.com" />
          </div>
          ...
          <div>
            <label className="mb-1 block text-sm text-white/70">Guests</label>
          ...
          <motion.button whileTap={{ scale: 0.98 }} disabled={status !== 'idle'} className="mt-2 inline-flex items-center justify-center rounded-full bg-gradient-to-r from-amber-500 via-rose-500 to-red-600 px-6 py-3 font-semibold text-white disabled:opacity-60">
            {status === 'loading' ? 'Booking…' : status === 'done' ? 'Reserved ✓' : 'Reserve'}
          </motion.button>
        </form>
      </div>
      <div className="rounded-2xl border border-white/10 bg-white/5 p-6 text-white">
        <h3 className="mb-4 font-serif text-2xl">Hours & Details</h3>
